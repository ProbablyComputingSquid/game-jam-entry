		...
		if (player.exists() && weaponEquipped == "sword") {
			...
			if (shotgun.shells > 0 && !shotgun.reloading) {
				...
				for (let i = 0; i < 5; i++) {
					...
					dir = rotateVector(dir, Math.random() - 0.5)
					add([
						sprite("bullet"),
						scale(0.1),
						pos(player.pos),
...
// collision handlers for upgrade price text
	...
	onUpdate("shotgun-magazine-upgrade", (upgrade) => {
	...
	function shopMenu() {
		...
		let health_upgrade_dialogue = [
			...
			{
				speaker: "Zen - Shopkeeper",
				text: "I hear that the orange enemies drop pineapples when they die. Maybe you could eat those."
			},
		]
...
// shopkeeper contact listeners
	...
	function waveDone() {
		const numberOfCoins = get("coin").length
		debug.log("+" + numberOfCoins + " coins")
		destroyAll("coin")
			...
			onCollideEnd("player", "glen", () => {
				...
				glen.onUpdate(() => {
					if (glen.pos.dist(glenOriginalPos) > 1) {
						...
						glen.onUpdate(() => {
						}); // Stop updating once Glen reaches the original position
					}
				});
			})
